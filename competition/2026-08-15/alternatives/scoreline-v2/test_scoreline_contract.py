#!/usr/bin/env python3
"""Contract tests for the AgentFit scoreline-v2 alternative deck.

These tests run strictly against competition/2026-08-15/alternatives/scoreline-v2/
and never touch the frozen submission/ directory or canonical docs.
"""
...
PPTX = ROOT / "agentfit-scoreline-v2.pptx"
PDF = ROOT / "agentfit-scoreline-v2.pdf"
...
class ScorelineV2ContractTest(unittest.TestCase):
    def test_directory_is_isolated_from_submission(self) -> None:
        ...
        for path in ROOT.rglob("*"):
            self.assertTrue(
                ...
                f"stray file outside scoreline-v2: {path}",
            )
    ...
    def test_introduction_under_five_hundred_non_whitespace_chars(self) -> None:
        ...
        for term in ("AgentFit", "AgentTeams", "OpsPilot", "拒绝自动化"):
    ...
    def test_validator_encodes_scoreline_contract(self) -> None:
    ...
    def test_validator_rejects_raster_media_and_transitions(self) -> None:
        validator = _load_validator()
        with tempfile.TemporaryDirectory(prefix="scoreline-native-") as temp_dir:
            temp = Path(temp_dir)
    ...
    def test_validator_flags_ml_term_on_first_four_slides(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="scoreline-ml-ban-") as temp_dir:
    ...
    def test_validator_flags_missing_page_title(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="scoreline-title-") as temp_dir:
    ...
    def test_validator_rejects_seventeen_blank_pdf_pages(self) -> None:
        validator = _load_validator()
        self.assertTrue(PPTX.is_file())
        with tempfile.TemporaryDirectory(prefix="scoreline-pdf-") as temp_dir:
