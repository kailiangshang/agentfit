#!/usr/bin/env python3
"""Compile the HTML-first AgentFit fusion-v3 deck into editable PPTX shapes."""
...
OUTPUT_PATH = ROOT / "agentfit-fusion-v3.pptx"
EXPECTED_SLIDES = 17
...
def build_deck(output_path: Path = OUTPUT_PATH) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="fusion-v3-pptx-") as temp_dir:
        temp = Path(temp_dir)
        base = temp / "base.pptx"
        ...
        payload["ops"].append(
            {
                "op": "set-props",
                "title": "AgentFit 融合版（fusion-v3）",
                "subject": "OpsPilot baseline 作首个 ProjectCase 的通用方案建筑师路演",
                "author": "AgentFit Team",
                "keywords": "AgentFit, AgentTeams, OpsPilot, fusion-v3, HTML-first",
