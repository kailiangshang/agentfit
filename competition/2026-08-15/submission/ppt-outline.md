...
叙事原则：前 4 页坚持业务问题、官方案例、材料编译和样本定义，不谈 ML/NAS；第 5–7 页用“样本构建、批量试验、误差分析和验证停止”的机器学习工程纪律解释完整方案空间、五阶段闭环和责任流水线；A1 再给出严格候选表示与七层映射。该类比不是产品名称，也不声称训练系统、AutoML 或反向传播，自动优化器也不是当前能力。OpsPilot 只作官方案例锚点，不作 AgentFit 运行证据。
...
| 5 | 调整对象是完整 Agent Solution，而不是 Agent 数量。 | 完整方案空间有哪些变量，如何控制复杂度 |
| 6 | 五阶段闭环：把机器学习的工程纪律带入 Agent 方案设计。 | 如何以样本、批量试验、Trace 和新案例收敛 |
...
- 机器学习工程纪律是第 5–7 页的解释桥梁，A1 承载严格技术映射；本轮不主张训练、反向传播、自动优化或 Meta-learning。
