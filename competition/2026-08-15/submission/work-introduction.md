...
企业提供业务材料、代表性案例和优先级，却常不知道是否该自动化、该用哪些 Agent 与能力。AgentFit 是运行在 AgentTeams 上的 Agent 方案建筑师：先把材料与案例转成验收约束，从最简单方案开始运行；依据证据调整 Tool、Skill、MCP、Memory、模型、Agent 拓扑和 Human 边界；再用新案例验证，交付最小充分的已验证方案包，也允许保留人工或拒绝自动化。

OpsPilot 是官方案例锚点，仅作材料与设计参考。retail/airline 的 DeepSeek + OpenCode 本地探索使用自建工具和代理评估器，只是探索性 Demo，不是官方成绩、AgentFit Candidate 或正式运行。真实 AgentFit 运行仍为 `NOT_STARTED`。
