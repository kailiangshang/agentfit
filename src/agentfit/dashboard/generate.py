...
h2{margin:0 0 10px;font-size:15px;color:#74d0c7;font-family:monospace}table{width:100%;border-collapse:collapse;font-size:12.5px}
...
.runtime-line{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px}.flow{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}
...
function outcomeCounts(items){const count={PASS:0,FAIL:0,ERROR:0};items.forEach(item=>{if(item.result in count)count[item.result]++;});return count.PASS+' PASS / '+count.FAIL+' FAIL / '+count.ERROR+' ERROR';}
...
add(overview,overviewKpis,el('div','status',ACCEPTANCE_STATUS));app.appendChild(overview);

const linkage=el('section','wide');linkage.appendChild(el('h2',null,'② 真实联动效果'));
const runtime=(DATA.run&&DATA.run.runtime_provenance)||{};const runtimeLine=el('div','runtime-line');['平台 '+(runtime.platform||'未记录'),'模型 '+(runtime.model_ref||'未记录'),'边界 '+(runtime.execution_boundary||'未记录'),'模式 '+(runtime.binding_mode||'未记录')].forEach(value=>runtimeLine.appendChild(badge(value)));linkage.appendChild(runtimeLine);
const trainingEvidence=DATA.training_evidence||[];const forward=trainingEvidence.filter(item=>item.phase==='forward');const candidateEvaluation=trainingEvidence.filter(item=>item.phase==='candidate_evaluation');const lossItems=Object.values(DATA.loss_traces||{}).flat();const firstLoss=lossItems[0]||{};const committed=((DATA.summary&&DATA.summary.transactions_committed)||[]).flatMap(item=>item.changes||[]);const firstChange=committed[0]||{};
if(trainingEvidence.length){const flow=el('div','flow');add(flow,flowStep('1 · 初始候选',outcomeCounts(forward),'Candidate '+shortRef(forward[0]&&forward[0].candidate_ref)),flowStep('2 · Trace 归因',(firstLoss.root_cause_layer||'—')+' · '+(firstLoss.failure_mode||'无失败'),(firstLoss.sample_id||'—')+' · 置信度 '+Number(firstLoss.confidence??0).toFixed(2)),flowStep('3 · 方案更新',(firstChange.layer||'—')+' / '+(firstChange.action||'无更新'),firstChange.element||'—'),flowStep('4 · 更新后回归',outcomeCounts(candidateEvaluation),'Candidate '+shortRef(candidateEvaluation[0]&&candidateEvaluation[0].candidate_ref)));linkage.appendChild(flow);}else linkage.appendChild(el('div','mut','本次 RunStore 没有训练前后 Episode。'));
app.appendChild(linkage);

const evidence=el('section','wide');evidence.appendChild(el('h2',null,'③ 逐样本最终证据'));
const finalEvidence=DATA.evaluation_evidence||[];if(finalEvidence.length){const rows=finalEvidence.map(item=>[item.purpose||'—',item.sample_id,resultBadge(item.result),item.run_index,item.error_code||(item.route||[]).join(' → ')||'—',code(shortRef(item.candidate_ref))]);const wrap=el('div','evidence-wrap');wrap.appendChild(table(['集合','样本','结果','RunIndex','实际路径 / 运行错误','Candidate'],rows));evidence.appendChild(wrap);}else evidence.appendChild(el('div','mut','最终评价尚未运行；当前只展示 adaptation 学习证据。'));app.appendChild(evidence);
...
acceptance.appendChild(table(['集合','通过率','PASS / FAIL / ERROR','成本','风险事件','验收门槛','结果'],acceptanceRows));const failures=(DATA.acceptance&&DATA.acceptance.failures)||[];if(failures.length)acceptance.appendChild(el('div','bad','未满足条件：'+failures.join(' · ')));app.appendChild(acceptance);
