...
section{background:#132f47;border:2px solid #28516d;border-radius:14px;padding:16px 18px}section.wide{grid-column:1/-1}
...
if(DATA.summary){const valid=typeof DATA.summary.final_pass_rate==='number';overviewKpis.appendChild(kpi(valid?(DATA.summary.final_pass_rate*100).toFixed(0)+'%':'—',valid?'训练批次通过率':'训练批次通过率 · 无有效方案评测'));overviewKpis.appendChild(kpi('v'+DATA.summary.final_solution_version,'方案版本'));overviewKpis.appendChild(kpi('$'+DATA.summary.total_cost_usd,'总成本'));overviewKpis.appendChild(kpi(DATA.summary.log_chain_valid?'✓':'✗','哈希链'));overviewKpis.appendChild(kpi(DATA.summary.converged?'收敛':'进行中','状态'));}
